...
async function run_db_connection() {
  try {
    ...
    app.get("/search_results", async (req, res) => {
      ...
      const incidentsFound = await incidents_col.find({ description: { $regex: searchQuery, $options: "i" } }).toArray();

      res.render("search_results", { incidentsFound, username: req.session.username, searchQuery });
    });
